...
import { isAfter, parseISO, startOfDay, subDays, subMonths, subYears } from 'date-fns';
  ...
  const barData = useMemo(() => {
    ...
    pastTransactions.forEach(txn => {
      ...
      if (tDate >= startDate && tDate <= today) {
        ...
        let key = tDate.toISOString().split('T')[0]; // default YYYY-MM-DD
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-2xl p-6 text-white shadow-lg lg:col-span-1 flex flex-col">
        ...
        <div className="border-t border-emerald-600/50 pt-4 flex-1 flex flex-col min-h-0">
          <h3 className="text-emerald-100 font-medium text-sm mb-3">Recent Transactions</h3>
          <div className="overflow-y-auto pr-1 space-y-2 max-h-48">
            {transactions.slice().sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(txn => (
              <div 
                key={txn.id}
              ...
              >
                <div className="min-w-0 flex-1 mr-4">
                  ...
                  <div className="text-xs text-emerald-200/70">{txn.date} &bull; {txn.type}</div>
                </div>
                <div className={`font-semibold text-sm whitespace-nowrap ${txn.type === 'Income' ? 'text-emerald-100' : 'text-rose-200'}`}>
                  {txn.type === 'Income' ? '+' : '-'}₱{txn.amount.toLocaleString(undefined, {minimumFractionDigits: 2})}
                </div>
              </div>
            ))}
            {transactions.length === 0 && (
              <div className="text-emerald-200/70 text-sm text-center py-2">No transactions yet</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 lg:col-span-1 flex flex-col">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to="/calendar" className="text-emerald-500 text-sm font-medium hover:underline">View All</Link>
        </div>
        <div className="flex-1 overflow-y-auto space-y-4">
          {upcomingPayments.length > 0 ? upcomingPayments.slice(0, 4).map(payment => (
            <div key={payment.id} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
              <div>
                <div className="font-semibold text-slate-800 dark:text-slate-200">{payment.name}</div>
                <div className="text-xs text-slate-500">Due: {payment.date}</div>
              </div>
            ...
            <div className="text-slate-500 text-center py-4">No upcoming payments</div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 lg:col-span-3">
        ...
        <div className="h-72">
          {barData.length > 0 ? (
            ...
            <div className="h-full flex items-center justify-center text-slate-500">No data for selected period</div>
          )}
        </div>
      </div>
